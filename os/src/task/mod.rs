...
pub struct TaskManager {
    ...
    inner: UPSafeCell<TaskManagerInner>,
     /// 记录每个任务每个系统调用的调用次数（用 UPSafeCell 包裹以支持全局静态可变）
...
impl TaskManager {
    ...
    pub fn increase_sys_call(&self, syscall_id: usize) {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
        let mut syscall_counter = self.syscall_count.exclusive_access();
        syscall_counter[current][syscall_id] += 1;
    }

    /// 查询当前任务某系统调用次数
    pub fn get_sys_call_times(&self, syscall_id: usize) -> usize {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
        let syscall_counter = self.syscall_count.exclusive_access();
